...
for entryIndex = 1:numEntries
    ...
    for t = 1:simulationDuration
        ...
        energyDrained = randi([0, 50], 1, numNodes);
        energyConsumed = randi([0, 100], 1, numNodes);
        if rand() < 0.5
            transmissionRange = 30;  % Default value
        else
            transmissionRange = randi([20, 40]);  % Vary transmission range within a range
        end
        % IPS detection logic
        ...
        if t <= attackDuration
            for i = 1:numNodes
                ...
                if isNodeInRange(nodePositions, i, attackNode, transmissionRange)
                    ...
                    preventedDos = preventDosAttack(i, nodePositions, attackNode, transmissionRange, energyLevels, energyThreshold);

                    if preventedDos && bypassTwo(energyLevels, transmissionCount, transmissionCountThreshold)
                    ...
                    transmissionCount = transmissionCount + randi([0, 5], 1, numNodes);
                end
            end
        end

        energyLevels = max(energyLevels - energyConsumed + energyDrained, 0);
    end
end
...
function [energyConsumed, energyDrained] = simulateDosEnergyConsumption(initialEnergy, currentEnergy, packetsIntercepted)
    ...
    interceptedFactor = 1 + 0.1 * packetsIntercepted; 
    energyConsumed = min(currentEnergy, initialEnergy * interceptedFactor) * 0.9;  
