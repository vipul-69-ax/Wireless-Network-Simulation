...
sinkholeResults = table('Size', [0, 10], 'VariableNames', {'EntryIndex','Node', 'AttackType', 'TimeStep', 'InitialEnergy', 'EnergyLevel', 'EnergyDrained', 'ActualEnergyConsumed', 'ActualAttackSuccessRate', 'AttackDuration'}, 'VariableTypes', {'double','double', 'char', 'double', 'double', 'double', 'double', 'double', 'double', 'double'});
        ...
        if t <= attackDuration
            for i = 1:numNodes
                ...
                if isNodeInRange(nodePositions, i, attackNode, transmissionRange)
                    ...
                    packetsIntercepted = simulateDosTraffic(nodePositions, i, attackNode, transmissionRange);
                    [energyConsumed, energyDrained] = simulateDosEnergyConsumption(initialEnergy, energyLevels(i), packetsIntercepted);
                    ...
                    newRow = createSimulationRow(entryIndex, i, attackType, t, initialEnergy, energyLevels(i), energyDrained, energyConsumed, actualAttackSuccessRate, attackDuration);
...
function packetsIntercepted = simulateDosTraffic(nodePositions, attackingNode, targetNode, transmissionRange)
    % Simulate a DoS attack by overwhelming the targeted node with high traffic
    packetsIntercepted = sum(isNodeInRange(nodePositions, attackingNode, targetNode, transmissionRange));
end

function preventedDos = preventDosAttack(node, nodePositions, attackNode, transmissionRange, energyLevels, energyThreshold)
    % IPS logic: Prevent DoS attack by checking if the attacker is in range and energy is below a threshold
    preventedDos = isNodeInRange(nodePositions, node, attackNode, transmissionRange) && (energyLevels(node) < energyThreshold);
end

function [energyConsumed, energyDrained] = simulateDosEnergyConsumption(initialEnergy, currentEnergy, packetsIntercepted)
    % Simulate energy consumption for DoS attack (consider intercepted packets)
    interceptedFactor = 1 + 0.1 * packetsIntercepted; % Adjust the factor based on intercepted packets
    energyConsumed = min(currentEnergy, initialEnergy * interceptedFactor) * 0.9;  % Default factor for DoS
...
function newRow = createSimulationRow(entryIndex,node, attackType, timeStep, initialEnergy, energyLevel, energyDrained, energyConsumed, attackSuccessRate, attackDuration)
    newRow = {entryIndex,node, attackType, timeStep, initialEnergy, energyLevel, energyDrained, energyConsumed, attackSuccessRate, attackDuration};
end
